...
# ---------------------------
# server.ps1
# Standalone module for editing the Game.ini / GameUserSettings.ini / run.json sources under config/ini:
...
# Dot-sourced by App.ps1: . (Join-Path $PSScriptRoot "settings\server.ps1")
# Exposes Show-ServerSettingsDialog -ConfigJsonPath <path>
# ---------------------------
...
function Show-ServerSettingsDialog {
    ...
    $root = Split-Path -Path $ConfigJsonPath -Parent
    $iniRoot = Join-Path $root 'config\ini'

    # path -> file content, populated on first access and written back on Save
    # (uses $script: so nested helper functions below can share it; dot-sourcing means
    # this actually lives in App.ps1's script scope, hence the unique/prefixed name)
    $script:serverSettingsIniCache = @{}
    ...
    function Get-IniContent {
        ...
        if ($script:serverSettingsIniCache.ContainsKey($Path)) {
            return $script:serverSettingsIniCache[$Path]
        }
        ...
        $script:serverSettingsIniCache[$Path] = $content
        return $content
    }
    ...
    function Save-IniCache {
        ...
        $deleted = 0
        $errors = 0
        foreach ($path in @($script:serverSettingsIniCache.Keys)) {
            try {
                if ([string]::IsNullOrWhiteSpace($script:serverSettingsIniCache[$path])) {
                ...
                Set-Content -LiteralPath $path -Value $script:serverSettingsIniCache[$path] -Encoding UTF8 -NoNewline -ErrorAction Stop
                $saved++
            } catch {
                $errors++
    ...
    function New-IniTextBox {
        ...
        $txt.Add_TextChanged({
            param($s, $e)
            $script:serverSettingsIniCache[[string]$s.Tag] = $s.Text
        })
        return $txt
    }
    ...
    $lblIniStatus = New-Object System.Windows.Forms.Label
    $lblIniStatus.Location = New-Object System.Drawing.Point(15, 10)
    $lblIniStatus.Size = New-Object System.Drawing.Size(650, 20)
    $lblIniStatus.Text = ''
    $pnlBottom.Controls.Add($lblIniStatus)
    ...
    $btnSave.Add_Click({
        ...
        $lblIniStatus.Text = "Saved at $(Get-Date -Format 'HH:mm:ss') - $($result.Saved) file(s) saved, $($result.Deleted) empty file(s) removed, $($result.Errors) error(s)."
    })
